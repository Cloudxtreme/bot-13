#
Hubot dependencies {
    Robot, Adapter, TextMessage, EnterMessage, LeaveMessage, Response
} = require 'hubot'

#
Custom Response class that adds a sendPrivate method
class IrcResponse extends Response
sendPrivate: (strings...) - > @robot.adapter.sendPrivate@ envelope,
strings...
...
self = @
    getMsg: (message) - > #console.log(message.user.name)# message.text = "noIdentify";#
console.log(message)@ bot.whois message.user.name, (text) - >
    console.log(text)
if (text.nick == 'NickServ'
    and text.host = 'services.') or(text.nick.toLowerCase() == text.account.toLowerCase() and text.accountinfo == 'is logged in as')
console.log('validated user: ' + text.nick)
self.receive message
else
    console.log('invalid user: ' + text.nick)
if (text.nick != 'intuxicated')
    message.text = self.robot.name + " noIdentify"
self.receive message


send: (envelope, strings...) - > #Use@ notice
if SEND_NOTICE_MODE is set
...
for str in strings@ bot.say target,
str

sendPrivate : (envelope, strings...) - > #Remove the room from the envelope and send as private message to user
...
topic : (envelope, strings...) - >
    data = strings.join " / "
channel = envelope.room@ bot.send 'TOPIC',
channel,
data

emote: (envelope, strings...) - > #Use@ notice
if SEND_NOTICE_MODE is set
return@ notice envelope,
strings
if process.env.HUBOT_IRC_SEND_NOTICE_MODE ?
...
for str in strings@ bot.action target,
str
...
for str in strings@ send envelope.user,
...
self.getMsg new EnterMessage(selfUser)
...
selfUser = self.getUserFromName self.robot.name
self.getMsg new LeaveMessage(selfUser)
...
getUserFromId : (id) - > #TODO : Add logic to convert object
if name matches
...
kick : (channel, client, message) - > @bot.emit 'raw',
command: 'KICK'
nick: process.env.HUBOT_IRC_NICK
args: [channel, client, message]

command: (command, strings...) - > @bot.send command,
strings...
...
# Override the response to provide a sendPrivate method@ robot.Response = IrcResponse

@ robot.name = options.nick
bot = new Irc.Client options.server,
options.nick,
client_options
...
bot.addListener 'notice',
(from, to, text) - >
if from is 'NickServ'
and text.toLowerCase().indexOf('identify') isnt - 1
bot.say 'NickServ',
"identify #{identify_args}"
else if options.nickpass and from is 'NickServ'
and(text.indexOf('Password accepted') isnt - 1 or text.indexOf('identified') isnt - 1)
for room in options.rooms@ join room

if options.connectCommand ?
    bot.addListener 'registered',
(message) - > #The 'registered'
event is fired when you are connected to the server
strings = options.connectCommand.split " "
self.command strings.shift(),
strings...

bot.addListener 'names',
(channel, nicks) - >
for nick of nicks
self.createUser channel,
nick
...
logger.info('Ignoring user: %s', from)# we 'll ignore this message if it'
s from someone we want to ignore
return

logger.info "NOTICE from #{from} to #{to}: #{message}"

user = self.createUser to,
from
self.getMsg new TextMessage(user, message)

bot.addListener 'message',
(from, to, message) - >
if options.nick.toLowerCase() == to.toLowerCase()# this is a private message,
let the 'pm'
listener handle it
return
...
logger.info('Ignoring user: %s', nick)# we 'll ignore this message if it'
s from someone we want to ignore
return
...
self.getMsg new TextMessage({
    reply_to: nick,
    name: nick
}, message)

bot.addListener 'join',
(channel, who) - >
    logger.info('%s has joined %s', who, channel)
user = self.createUser channel,
who
user.room = channel
self.getMsg new EnterMessage(user)

bot.addListener 'part',
(channel, who, reason) - >
...
bot.addListener 'quit',
(who, reason, channels) - >
    logger.info '%s has quit: %s (%s)',
who,
channels,
reason
for ch in channels
...
# as of hubot 2.4.2,
the first param to send() is an object with 'user'#
and 'room'
data inside.detect the old style here.
if envelope.reply_to
user = envelope
else# expand envelope
user = envelope.user
room = envelope.room

if user# most common
case -we 're replying to a user in a room
      if user.room
        target = user.room
      # reply directly
      else if user.name
